##### load ZF model with corresponding bestfit
try(setwd(dirname(rstudioapi::getSourceEditorContext()$path)))
source("compileZF_2040.R")
source("modelZF_2040.R")
fitlist <- c()
for (k in 0:39){
...
k <- 1
  ...
  #reactionlist$Rate <- replaceSymbols(c("SOX"), c("(sox2 + sox3 + sox19a + (1-MZsox19b)*kSUM_sox19b*sox19b)"), as.character(reactionlist$Rate))
...
fitModelTG_corrected <- function(mytarg="ENSDART00000113803.4", j=1){
  ...
  trafoL <- branch(trafo, covtable) %>%
    ...
    #insert("target~target_condition", Experiment!="WT", target = c("nanog"), condition=Experiment) %>%
  ...
  for (C in conditions) {
    ...
    myx <- myx + Xs(mymodel2,
                ...
                #optionsSens = list(method = "bdf", rtol = tolerances, atol = tolerances),
  ...
  #print(mypouter)
  #print(myp0(mypouter))
  # print((myx*myp0)(mytimes, mypouter, deriv=F))
  # print(myobj(mypouter))
  ...
  # fitlist <- fitlist[order(fitlist$value),]
  # head(fitlist)
  # bestfit2 <- unlist(fitlist[1,-c(1:4)])
  # prediction <- (g*x*p0)(times=seq(0,4,len=100), bestfit2, fixed=NULL, deriv=FALSE)
  # plotCombined(prediction, data, name=="TG", facet="grid")
}
